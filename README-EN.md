...
The module operates by loading data exclusively from two specific compendiums inside the `_renderHTML()` method. This ensures that only the official features and the custom ones specifically created by the user are imported into the NPC Creator:
...
graph TD
    ...
    C --> D[Separate and catalog documents by type: npc_class, npc_template, npc_feature]
    D --> E[Render Reactive Creator UI]
```
...
*   `module.json`: The module manifest indicating the ESModule script dependency, CSS styles, compatibility with Foundry VTT V13+, definition of the empty compendium, and recommendation of the `lancer-npcs-basico` package.
*   `scripts/npc-creator.mjs`: Contains all the rendering logic, event listeners, NPC stat calculation, and smart compendium loading.
